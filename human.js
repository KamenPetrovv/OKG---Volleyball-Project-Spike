...
var colors = [0xeac086,0xeac086,0xeac086,0xeac086,0xeac086,0xeac086]; // [глава,обувка,таз,сферички,крайник,торс,]
var headScale = 1.0;
...
{
	return parametricImage(texLimb,colors[2],function (u,v)
	{
...
function формаКрайник(params)
{
	...
	var image = parametricImage(texLimb,colors[4], function (u,v)
	{
		v = 360*v;
...
function формаТорс(params)
{
	...
	var image = parametricImage(texLimb,colors[5], function (u,v)
	{
		var r = offset+scale*cos(alpha+dAlpha*u);
...
function човек()
{
	...
	shape.moveTo( -10,0 );
	shape.lineTo( -9, height);
	shape.quadraticCurveTo( -8, height - 2,-3,height + 1);
	shape.quadraticCurveTo(4,height - 5 , 9, height - 4 );
	shape.lineTo( 10, 1 );
	shape.quadraticCurveTo( 0,-1,-10, 0 );
	...
	var shoes = new THREE.Mesh( geometry, material ) ;
	shoes.rotation.z = -Math.PI/2;
	shoes.rotation.x = Math.PI;
	shoes.position.y += 2.6;
	shoes.position.z += 1.6;
	shoes.position.x -= 0.5;
	var s = 0.45;
	shoes.scale.x = s;
	shoes.scale.y = s;
	shoes.scale.z = s;
	...
	var shape = new THREE.Shape();
	shape.moveTo( -4,0 );
	shape.lineTo( -6, height);
	shape.lineTo( -3, height + 1);
	shape.quadraticCurveTo( 0, height - 2,3,height + 1);
	shape.lineTo( 6, height );
	shape.lineTo( 4, 0 );
	shape.lineTo( -4, 0 );
	...
	var geometry = new THREE.ExtrudeGeometry( shape, extrudeSettings );
	var material = new THREE.MeshLambertMaterial( { color: 0x0F68DB } );
	var Tshirt = new THREE.Mesh( geometry, material ) ;

	Tshirt.position.y -= 0.4;
	Tshirt.rotation.y = Math.PI/2;
	Tshirt.position.x -= 3;

	//Ръкави

	var geometry = new THREE.CylinderGeometry( 3, 2, 7, 100,1,true);
	var material = new THREE.MeshLambertMaterial({color:0x0F68DB});
	var sleeve = new THREE.Mesh( geometry, material );
	sleeve.position.y += 3;
	...
	var geometry = new THREE.CylinderGeometry( 3.5, 2.4, 11, 100,1,true);
	var material = new THREE.MeshLambertMaterial({color:0x06143B});
	var cylinder = new THREE.Mesh( geometry, material );
	cylinder.position.y -= 0.3;

	//Гащи

	var geometry = new THREE.SphereGeometry( 4.7, 50, 50 );
	geometry.applyMatrix( new THREE.Matrix4().makeScale( 0.9, 1.1, 1.5 ) );
	var material = new THREE.MeshLambertMaterial( {color: 0x06143B} );
	...
	obj.л_крак = става(obj.таз,[0,-3,-4],[0,180,200],[4,15,4,-70,220,1,0.3,2],формаКрайник);
		obj.л_крак.children[0].add( cylinder );
	...
	obj.д_ръка = става(obj.тяло,[0,14,feminine?5:6],[-10,180,-180],[3.5,11,2.5,-90,360,0.9,0.2,1.5],формаКрайник);
		obj.д_ръка.children[0].add(sleeve.clone());
